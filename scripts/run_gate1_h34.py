"""
Gate 1 Backtest Runner — H34 RSI(2) Oversold SPY Mean Reversion
QUA-259 | Backtest Runner Agent | 2026-03-16
...
Runs full Gate 1 pipeline:
  ...
  - Parameter sensitivity (RSI threshold 5, 10, 15)
  - Special validations: 2022 regime analysis, GFC 2008–2009 MDD, 2022 MDD
  - DSR computation
  - Saves to backtests/H34_RSI2OversoldSPY_{date}.json and _verdict.txt
"""
...
from h34_rsi2_oversold_spy import run_backtest, PARAMETERS, download_data
from oos_data_quality import validate_oos_data, OOSDataQualityError

STRATEGY_NAME = "H34_RSI2OversoldSPY"
...
def run_sensitivity(param_name: str, values: list) -> dict:
    """Scan IS Sharpe across param variations (±20% variation on RSI threshold)."""
...
def run_special_validations(is_result: dict, oos_result: dict) -> dict:
    """
    Special validations required by QUA-259:
    1. 2022 regime analysis (should be mostly in-cash)
    2. GFC 2008–2009 MDD
    ...
    # 1. 2022 regime analysis — from OOS backtest daily_df
    oos_daily = oos_result.get("daily_df", pd.DataFrame())
    if not oos_daily.empty and "regime_active" in oos_daily.columns:
        mask_2022 = oos_daily.index.year == 2022
        if mask_2022.any():
            regime_2022 = oos_daily.loc[mask_2022, "regime_active"]
            in_cash_pct_2022 = float((~regime_2022.astype(bool)).mean())
            svs["regime_2022_in_cash_pct"] = round(in_cash_pct_2022, 4)
            svs["regime_2022_pass"] = in_cash_pct_2022 > 0.5  # mostly in-cash during bear market
            print(f"  2022 in-cash (regime off): {in_cash_pct_2022:.1%} — {'PASS' if svs['regime_2022_pass'] else 'NOTE'}")
        else:
            svs["regime_2022_in_cash_pct"] = None
            svs["regime_2022_pass"] = None
    else:
        svs["regime_2022_in_cash_pct"] = None
        svs["regime_2022_pass"] = None
...
GATE1_CRITERIA = {
    ...
    "Trade count >= 100 (IS)": lambda m: m["trade_count"] >= 100,
    "MC p5 Sharpe >= 0.5": lambda m: not np.isnan(m.get("mc_p5_sharpe", np.nan)) and m.get("mc_p5_sharpe", 0) >= 0.5,
...
def main():
    ...
    try:
        ...
        print(f"  IS Regime active: {is_result['regime_pct']:.1%}")
    except Exception as e:
        print(f"[FATAL] IS backtest failed: {e}")
        traceback.print_exc()
        sys.exit(1)

    # ── 2. OOS Data Download (for DQ check) ────────────────────────────────
    print(f"\n[DQ] Downloading OOS data for quality check ({OOS_START} to {OOS_END})...")
    try:
        oos_data = download_data("SPY", OOS_START, OOS_END, PARAMETERS["sma_period"])
        oos_ts = pd.Timestamp(OOS_START)
        oos_data = oos_data.loc[oos_data.index >= oos_ts]
    ...
    try:
        ...
        print(f"  OOS Regime active: {oos_result['regime_pct']:.1%}")
    except Exception as e:
    ...
    oos_metrics_for_dq = {
        ...
        "post_cost_sharpe": oos_result["sharpe"],  # post-cost already in simulation
    ...
    # Assume $25k portfolio buying SPY at ~$400 → ~62 shares
    avg_spy_price = 400.0
    ...
    spy_closes = is_daily["equity"].values if not is_daily.empty else np.array([])
    # Use equity curve as proxy for prices in permutation test
    ...
    print("\n[SENS] Parameter sensitivity: RSI entry threshold (5, 10, 15)...")
    sens_results = run_sensitivity("rsi_entry_threshold", [5, 10, 15])
    ...
    # ── 9. Post-cost Sharpe (already embedded in simulation) ────────────────
    # Transaction costs are applied in the strategy simulation itself
    post_cost_sharpe_is = is_result["sharpe"]   # costs already included
    post_cost_sharpe_oos = oos_result["sharpe"]  # costs already included
    ...
    metrics = {
        ...
        "market_impact_bps": round(mi_results["market_impact_bps"], 4) if mi_results["market_impact_bps"] is not None and not np.isnan(mi_results["market_impact_bps"]) else None,
        "liquidity_constrained": mi_results["liquidity_constrained"],
        "order_to_adv_ratio": round(mi_results["order_to_adv_ratio"], 8) if mi_results["order_to_adv_ratio"] is not None and not np.isnan(mi_results["order_to_adv_ratio"]) else None,
        ...
        "sensitivity_max_pct_change": sens_results["max_pct_change"],
        "sensitivity_scan": sens_results["scan_results"],

        # Special validations
        "special_validations": spec_validations,

        # Flags
        "look_ahead_bias_flag": False,  # 200-SMA uses shift(1), RSI computed prior-day
    ...
    if not oos_trades.empty:
        metrics["trade_log_oos"] = oos_trades.head(50).to_dict(orient="records")
    else:
        metrics["trade_log_oos"] = []

    # IS trade log sample
    is_trades = is_result["trades"]
    if not is_trades.empty:
        metrics["trade_log_is_sample"] = is_trades.head(20).to_dict(orient="records")
    else:
        metrics["trade_log_is_sample"] = []
    ...
    verdict_lines += [
        ...
        "Parameter Sensitivity (RSI threshold: 5, 10, 15)",
        "=" * 60,
        f"  Max Sharpe change:  {metrics['sensitivity_max_pct_change']:.1f}%  (threshold < 30%)  {'PASS' if metrics['sensitivity_pass'] else 'FAIL'}",
    ]
    for row in metrics["sensitivity_scan"]:
        if "error" not in row:
            verdict_lines.append(
                f"  rsi_threshold={row['param_value']}: Sharpe={row['sharpe']:.4f} ({row['pct_change_from_base']:.1f}% change)"
            )
    ...
    verdict_lines += [
        ...
        f"  2022 Regime Pass:   {spec_validations.get('regime_2022_pass', 'N/A')}",
        f"  GFC 2008-2009 MDD:  {spec_validations.get('gfc_2008_2009_mdd', 'N/A')}",
