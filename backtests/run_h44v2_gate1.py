"""
Gate 1 Backtest Runner: H44v2 LQD/IEF Credit Risk Appetite Timer — Trailing Stop Redesign
Engineering Director | QUA-343 | 2026-05-01

Changes from H44 Gate 1:
- Uses H44v2 strategy with 15% trailing stop-loss overlay (base case).
- Sensitivity: trailing stop at 12%, 15%, 18%.
- Re-entry cooldown: signal risk-on AND SPY >= 3% above stop trigger price.
- All other statistical tests (MC, bootstrap, WF, permutation) unchanged.

Full Gate 1 evaluation:
- IS backtest (2007-01-01 to 2021-12-31)
- OOS backtest (2022-01-01 to 2025-12-31)
- 4 walk-forward folds (expanding IS window)
- Trailing stop sensitivity (12%, 15%, 18%)
- Parameter sensitivity sweep (lookback ±20%, threshold ±20%)
- Monte Carlo Sharpe (1000 simulations)
- Block bootstrap 95% CI
- Permutation p-value (500 permutations)
- Walk-forward variance metrics
- GFC exit timing, 2022 monthly regime, sub-period decomposition
- Gate 1 verdict JSON + markdown report

Outputs:
  backtests/h44v2_gate1_results.json
  backtests/h44v2_gate1_report.md

Ref: QUA-343, QUA-342, QUA-341, QUA-339
"""
...
from strategies.h44v2_lqd_ief_credit_risk_appetite_timer import (
    run_backtest,
    PARAMETERS,
)
...
STRATEGY_NAME = "H44v2_LQD_IEF_CreditRiskAppetimeTimer_TrailingStop"
...
# Primary parameter set (15% trailing stop base case)
PRIMARY_PARAMS = PARAMETERS.copy()

# Trailing stop sensitivity: 12%, 15%, 18%
TRAILING_STOP_VARIANTS = [
    {"trailing_stop_pct": 0.12, "label": "12% trailing stop (tight)"},
    {"trailing_stop_pct": 0.15, "label": "15% trailing stop (base case)"},
    {"trailing_stop_pct": 0.18, "label": "18% trailing stop (loose)"},
]

# Signal parameter sensitivity grid
PARAM_GRID = [
...
def permutation_test_alpha(
    ...
    for _ in range(n_perms):
        ...
        s = perm_rets.mean() / (perm_rets.std() + 1e-8) * np.sqrt(TRADING_DAYS_PER_YEAR)
...
def compute_dsr(daily_returns: np.ndarray, n_trials: int) -> float:
    ...
    sigma_sr = math.sqrt(abs(
        (1 + 0.5 * sr**2 - skew * sr + (kurt / 4) * sr**2) / max(T - 1, 1)
    ))
...
def run_walk_forward(n_folds: int = 4) -> list:
    """
    4-fold expanding walk-forward over IS period (2007-2021).
    Fold 1: Train 2007-2009, OOS 2010-2012
    Fold 2: Train 2007-2012, OOS 2013-2015
    Fold 3: Train 2007-2015, OOS 2016-2018
    Fold 4: Train 2007-2018, OOS 2019-2021
    """
    fold_results = []
    block_boundaries = [
    ...
    for fold in range(n_folds):
        ...
        try:
            ...
            fold_results.append({
                ...
                "is_stop_exits":   is_res["n_stop_exits"],
                "oos_stop_exits":  oos_res["n_stop_exits"],
...
def run_trailing_stop_sensitivity() -> list:
    """Test IS performance at 12%, 15%, 18% trailing stop."""
    results = []
    for variant in TRAILING_STOP_VARIANTS:
        p = PRIMARY_PARAMS.copy()
        p["trailing_stop_pct"] = variant["trailing_stop_pct"]
        label = variant["label"]
        print(f"  Trailing stop sensitivity: {label}")
        try:
            ...
            results.append({
                ...
                "n_stop_exits":      res["n_stop_exits"],
                "trade_count":       res["trade_count"],
                "pct_in_spy":        _safe_float(res["pct_in_spy"]),
            })
        except Exception as e:
            results.append({"trailing_stop_pct": variant["trailing_stop_pct"],
...
def run_sensitivity_sweep() -> list:
    """Run IS backtest with 5 signal param variants (lookback ±20%, threshold ±20%)."""
    sweep_results = []
    for cfg in PARAM_GRID:
        label = (f"lb={cfg['lookback_days']}d,"
                 f"thresh={cfg['signal_threshold']:.4f},"
                 f"smooth={cfg['smoothing_days']}d")
        print(f"  Signal sensitivity: {label}")
        try:
            p = _build_params(cfg)
            res = run_backtest(IS_START, IS_END, p)
...
def gfc_exit_analysis() -> dict:
    try:
        ...
        return {
            ...
            "gfc_stop_exits":            gfc_res["n_stop_exits"],
        }
    except Exception as e:
        traceback.print_exc()
        return {"error": str(e)}
...
def oos_2022_monthly_analysis() -> dict:
    try:
        ...
        return {
            ...
            "stop_exits_2022":    res_2022["n_stop_exits"],
    ...
    for r_start, r_end, label in windows:
        try:
            ...
            print(f"  {label}: Sharpe={fmt(res['sharpe'])} "
                  f"transitions={res['n_transitions']} stop_exits={res['n_stop_exits']} "
                  f"MDD={pct(res['max_drawdown'])}")
        except Exception as e:
...
def main():
    ...
    print("=" * 70)
    print("H44v2 LQD/IEF CREDIT RISK APPETITE TIMER — GATE 1 BACKTEST RUNNER")
    print("Trailing Stop-Loss Redesign | QUA-343")
    print("=" * 70)
    print(f"IS window:  {IS_START} → {IS_END}")
    print(f"OOS window: {OOS_START} → {OOS_END}")
    print(f"Primary params: lookback=20d, threshold=0.0%, smoothing=1d, "
          f"trailing_stop=15%, reentry_margin=3%")
    print(f"Run date: {TODAY}")
    print()

    # ── 1. IS Backtest ─────────────────────────────────────────────────────────
    print("Step 1: IS backtest (2007–2021) with 15% trailing stop...")
    ...
    is_n_stop   = is_result["n_stop_exits"]

    print(f"  IS Sharpe={fmt(is_sharpe)}  MDD={pct(is_mdd)}  "
          f"Transitions={is_ntrans}  StopExits={is_n_stop}  "
          f"Trades(exits)={is_trades}  WinRate={pct(is_winrate)}  "
          f"PF={fmt(is_pf, 2)}  SPY%={pct(is_pct_spy)}")

    if is_ntrans < G1_MIN_TRADES:
        print(f"\n[FLAG] Transition count ({is_ntrans}) < {G1_MIN_TRADES} — Gate 1 FAIL on trade count.")
    if is_mdd is not None and is_mdd < G1_MDD:
        print(f"[FLAG] IS MDD ({pct(is_mdd)}) exceeds 20% threshold.")
    else:
        print(f"[PASS] IS MDD ({pct(is_mdd)}) is within 20% threshold — stop-loss working.")
    ...
    oos_n_stop   = oos_result["n_stop_exits"]

    print(f"  OOS Sharpe={fmt(oos_sharpe)}  MDD={pct(oos_mdd)}  "
          f"Transitions={oos_ntrans}  StopExits={oos_n_stop}  "
          f"Trades(exits)={oos_trades}  WinRate={pct(oos_winrate)}  "
    ...
    n_trials = len(PARAM_GRID) * 2 + len(TRAILING_STOP_VARIANTS)
    ...
    for r in wf_results:
        print(f"    Fold {r['fold']}: IS={fmt(r.get('train_sharpe'))} "
              ...
              f"stop_exits_oos={r.get('oos_stop_exits','?')} pass={r.get('fold_pass')}")
    ...
    # ── 10. Trailing Stop Sensitivity ────────────────────────────────────────
    print("\nStep 10: Trailing stop sensitivity (12%, 15%, 18%)...")
    trailing_stop_sens = run_trailing_stop_sensitivity()
    for ts in trailing_stop_sens:
        if "error" not in ts:
            print(f"  {ts['label']}: IS Sharpe={fmt(ts['is_sharpe'])} "
                  f"MDD={pct(ts['is_mdd'])} StopExits={ts.get('n_stop_exits','?')}")

    # ── 11. Signal Parameter Sensitivity Sweep ────────────────────────────────
    print("\nStep 11: Signal parameter sensitivity sweep...")
    sweep = run_sensitivity_sweep()
    sens_pass, sens_max_delta = sensitivity_pass(sweep, is_sharpe)
    print(f"  Signal sensitivity pass: {sens_pass}  (max_delta={pct(sens_max_delta)})")

    # ── 12. GFC Exit Timing ───────────────────────────────────────────────────
    print("\nStep 12: GFC 2008-2009 exit timing analysis...")
    gfc_analysis = gfc_exit_analysis()
    if "error" not in gfc_analysis:
        print(f"  GFC first cash date: {gfc_analysis['first_cash_date_2008']} | "
              f"Stop exits in GFC: {gfc_analysis.get('gfc_stop_exits', '?')} | "
              f"Exit before Oct 2008: {gfc_analysis['exited_before_oct_2008']}")

    # ── 13. 2022 Monthly Regime ───────────────────────────────────────────────
    print("\nStep 13: 2022 OOS monthly regime analysis...")
    analysis_2022 = oos_2022_monthly_analysis()
    if "error" not in analysis_2022:
        print(f"  2022: MDD={pct(analysis_2022['mdd_2022'])} "
              f"TotalRet={pct(analysis_2022['total_ret_2022'])} "
              f"StopExits={analysis_2022.get('stop_exits_2022','?')} "
              f"MDD_FLAG={analysis_2022['mdd_flag']}")

    # ── 14. Sub-period Sharpe Decomposition ───────────────────────────────────
    print("\nStep 14: Sub-period Sharpe decomposition (2007-2012, 2013-2018, 2019-2021)...")
    regime_results = regime_breakdown()
    ...
    print(f"\n{'=' * 70}")
    print(f"H44v2 GATE 1 VERDICT: {overall_verdict} ({n_pass}/{n_total} checks passed)")
    print(f"{'=' * 70}")
    ...
    results = {
        ...
        "oos_start":            OOS_START,
        "oos_end":              OOS_END,
        "trailing_stop_pct":    PRIMARY_PARAMS["trailing_stop_pct"],
        "reentry_margin":       PRIMARY_PARAMS["reentry_margin"],
        ...
        "n_stop_exits_is":      is_n_stop,
        "n_stop_exits_oos":     oos_n_stop,
        ...
        "trailing_stop_sensitivity": trailing_stop_sens,
        ...
        "look_ahead_bias_note": (
            ...
            "trailing stop checked at close T, exit fires at T+1 open; "
            "no look-ahead bias"
        ),
    }
    ...
    json_path = os.path.join(backtests_dir, "h44v2_gate1_results.json")
    ...
    md_path = os.path.join(backtests_dir, "h44v2_gate1_report.md")
...
def _save_blocked_result(dq_report: dict):
    backtests_dir = os.path.join(_REPO_ROOT, "backtests")
    os.makedirs(backtests_dir, exist_ok=True)
    path = os.path.join(backtests_dir, "h44v2_gate1_results.json")
    out = {
        "strategy_name": STRATEGY_NAME,
        "date": TODAY,
...
def _build_markdown_report(r: dict) -> str:
    ...
    trailing_stop = r.get("trailing_stop_pct", 0.15)
    reentry_margin = r.get("reentry_margin", 0.03)

    lines = [
        f"# H44v2 LQD/IEF Credit Risk Appetite Timer — Gate 1 Report",
        f"",
        f"**Strategy:** {r['strategy_name']}",
        f"**Version:** H44v2 (trailing stop-loss redesign — [QUA-343](/QUA/issues/QUA-343))",
        f"**Parent escalation:** [QUA-342](/QUA/issues/QUA-342) — CEO ruling: redesign with trailing stop",
        f"**Run date:** {r['date']}",
        f"**IS window:** {r['is_start']} → {r['is_end']}",
        f"**OOS window:** {r['oos_start']} → {r['oos_end']}",
        f"**Signal params:** lookback=20d, threshold=0.0%, smoothing=1d, riskoff=cash",
        f"**Stop-loss params:** trailing_stop={trailing_stop:.0%}, re-entry margin={reentry_margin:.0%}",
        f"",
        f"## Overall Verdict: {r['overall_verdict']}",
        f"",
        f"## H44v2 vs H44 Gate 1 Comparison",
        f"",
        f"| Metric | H44 Result | H44v2 Result | H44v2 Target | Status |",
        f"|--------|-----------|-------------|-------------|--------|",
        f"| IS Sharpe | 0.6822 | {_fv(r.get('is_sharpe'))} | >1.0 | {_pf((r.get('is_sharpe') or 0) > 1.0)} |",
        f"| IS Max Drawdown | -35.53% | {_pct(r.get('is_max_drawdown'))} | <20% | {_pf((r.get('is_max_drawdown') or -1) > -0.20)} |",
        f"| OOS Sharpe | 0.7687 | {_fv(r.get('oos_sharpe'))} | >0.7 | {_pf((r.get('oos_sharpe') or 0) > 0.7)} |",
        f"| WF folds passed | 0/4 | {r.get('wf_windows_passed', 'N/A')}/4 | ≥3/4 | {_pf((r.get('wf_windows_passed') or 0) >= 3)} |",
        f"| MC p5 Sharpe | 1.8274 | {_fv(r.get('mc_p5_sharpe'))} | ≥0.5 | {_pf((r.get('mc_p5_sharpe') or 0) >= 0.5)} |",
        f"",
        f"## Primary Metrics",
        f"",
        ...
        f"| Max Drawdown | {_pct(r.get('is_max_drawdown'))} | {_pct(r.get('oos_max_drawdown'))} | IS<20%, OOS<25% | IS:{_pf((r.get('is_max_drawdown') or -1) > -0.20)} OOS:{_pf((r.get('oos_max_drawdown') or -1) > -0.25)} |",
        ...
        f"| Stop-Loss Exits (IS) | {r.get('n_stop_exits_is', 'N/A')} | {r.get('n_stop_exits_oos', 'N/A')} | — | — |",
        ...
        f"| Signal sensitivity max delta | {_pct(r.get('sensitivity_max_delta'))} | <30% | {_pf(r.get('sensitivity_pass', False))} |",
        f"| Market impact | {_fv(r.get('market_impact_bps'), 2)} bps | — | — |",
        f"",
        f"## Walk-Forward Detail",
        f"",
        f"| Fold | Train Period | OOS Period | IS Sharpe | OOS Sharpe | Stop Exits OOS | Pass? |",
        f"|------|-------------|-----------|-----------|------------|----------------|-------|",
    ]
    ...
    for fold in r.get("wf_windows", []):
        lines.append(
            ...
            f"| {fold.get('oos_stop_exits', '?')} "
            f"| {fold.get('fold_pass', '?')} |"
        )

    lines += [
        f"",
        f"## Trailing Stop Sensitivity",
        f"",
        f"| Trailing Stop | IS Sharpe | IS MDD | Stop Exits | Transitions | % in SPY |",
        f"|--------------|-----------|--------|-----------|-------------|----------|",
    ]
    for ts in r.get("trailing_stop_sensitivity", []):
        if "error" not in ts:
            lines.append(
                f"| {ts.get('trailing_stop_pct', 0):.0%} ({ts.get('label', '').split('(')[1].rstrip(')')}) "
                f"| {_fv(ts.get('is_sharpe'))} "
                f"| {_pct(ts.get('is_mdd'))} "
                f"| {ts.get('n_stop_exits', 'N/A')} "
                f"| {ts.get('n_transitions', 'N/A')} "
                f"| {_pct(ts.get('pct_in_spy'))} |"
            )

    lines += [
        f"",
        f"## Signal Parameter Sensitivity Sweep",
        f"",
        f"| Lookback | Threshold | Smoothing | IS Sharpe | IS MDD | Stop Exits |",
        f"|----------|-----------|-----------|-----------|--------|------------|",
    ]
    for row in r.get("sensitivity_sweep", []):
        if "error" not in row:
            lines.append(
                ...
                f"| {row.get('n_stop_exits', 'N/A')} |"
            )

    lines += [
        f"",
        f"## GFC 2008-2009 Exit Analysis",
        f"",
    ]
    gfc = r.get("gfc_exit_analysis", {})
    if "error" not in gfc:
        exit_flag = "PASS: before Oct 2008" if gfc.get("exited_before_oct_2008") else "FAIL: NOT before Oct 2008"
        lines += [
            f"- First cash date (GFC window): **{gfc.get('first_cash_date_2008', 'N/A')}**",
            f"- Cash days (Sep–Oct 2008): **{gfc.get('cash_days_sep_oct_2008', 'N/A')}**",
            f"- Stop-loss exits in GFC period: **{gfc.get('gfc_stop_exits', 'N/A')}**",
    ...
    lines += [
        f"",
        f"## 2022 OOS Monthly Regime Analysis (Rate-Shock Year)",
        f"",
    ]
    ...
    if "error" not in a22:
        lines += [
            ...
            f"- 2022 Stop-loss exits: **{a22.get('stop_exits_2022', 'N/A')}**",
    ...
    lines += [
        ...
        f"",
        f"| Period | Sharpe | MDD | Win Rate | Transitions | Stop Exits | Total Return |",
        f"|--------|--------|-----|----------|-------------|------------|--------------|",
    ]
    for reg in r.get("regime_breakdown", []):
        if "error" not in reg:
            lines.append(
                ...
                f"| {reg.get('n_transitions', 'N/A')} "
                f"| {reg.get('n_stop_exits', 'N/A')} "
                f"| {_pct(reg.get('total_ret'))} |"
            )
    ...
    lines += [
        ...
        f"",
        f"---",
        f"*Generated by Engineering Director | [QUA-343](/QUA/issues/QUA-343) | {TODAY}*",
    ]
