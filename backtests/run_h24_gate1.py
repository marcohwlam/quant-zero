"""
H24 Gate 1 Backtest Runner — Combined IBS Mean Reversion + Turn-of-Month
QUA-227

IS period:  2018-01-01 to 2023-12-31 (6 years — includes COVID, rate-shock)
OOS period: 2024-01-01 to 2025-12-31 (strict temporal holdout)

Walk-forward expanding windows:
  W1: 2018–2019 train / 2020 test
  W2: 2018–2020 train / 2021 test
  W3: 2018–2021 train / 2022 test
  W4: 2018–2022 train / 2023 test
...
from strategies.h24_combined_ibs_tom import (
    run_backtest,
    PARAMETERS,
...
STRATEGY_NAME = "H24_CombinedIBSTOM"
OUTPUT_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "h24_combined_ibs_tom",
)

# Walk-forward expanding windows (IS expands; OOS is annual hold-out)
WF_WINDOWS = [
...
REGIME_SLICES = {
    ...
    "stimulus_era_2021":     ("2021-01-01", "2021-12-31"),
...
def run_sensitivity_scan(base_params: dict, start: str, end: str) -> dict:
    """Vary each free parameter independently (±20% per AGENTS.md spec)."""
    ...
    param_grid = {
        "ibs_entry_threshold": [0.20, 0.25, 0.30],       # ±20% of 0.25
        "ibs_exit_threshold":  [0.60, 0.75, 0.90],       # ±20% of 0.75
        "max_hold_days":       [2, 3, 4],                  # ±1 day
        "tom_entry_day":       [-3, -2, -1],               # ±1 day
        "tom_exit_day":        [2, 3, 4],                  # ±1 day
        "vix_threshold":       [24.0, 30.0, 36.0],        # ±20% of 30
    }
...
def run_regime_slices(params: dict) -> dict:
    print("\nRegime-slice analysis (4 regimes)...")
    results = {}
...
def compute_market_impact_spy(start: str, end: str, init_cash: float) -> dict:
    """SPY square-root market impact model."""
    try:
...
def build_verdict(metrics: dict) -> dict:
    ...
    # Regime criterion: IS Sharpe ≥ 0.8 in ≥ 2 of 4 regimes, ≥ 1 stress regime
    regime_slices = metrics.get("regime_slices", {})
    stress_regimes     = ["rate_shock_2022", "covid_crash_2020"]
    non_stress_regimes = ["pre_covid_2018_2019", "stimulus_era_2021"]
    stress_passing     = [r for r in stress_regimes
                          if regime_slices.get(r, {}).get("sharpe", 0.0) >= 0.8]
    nonstress_passing  = [r for r in non_stress_regimes
                          if regime_slices.get(r, {}).get("sharpe", 0.0) >= 0.8]
    regime_criterion_pass = (
        len(stress_passing) >= 1
        and (len(stress_passing) + len(nonstress_passing)) >= 2
    )
...
def main():
    print("=" * 70)
    print("H24 Combined IBS+TOM — Gate 1 Backtest Runner")
    print(f"Strategy: {STRATEGY_NAME}")
    print(f"IS:  {IS_START} → {IS_END}  (6 years)")
    ...
    # ── 1. IS Backtest ─────────────────────────────────────────────────────────
    print(f"\n[1/8] IS Backtest ({IS_START} → {IS_END})...")
    ...
    is_returns   = np.array(is_result["returns"].values
                            if isinstance(is_result["returns"], pd.Series)
                            else is_result["returns"])
    ...
    print(f"  IS: Sharpe={is_sharpe:.4f}, MDD={is_mdd:.2%}, "
          f"Trades={is_trades} ({is_tpy}/yr), WinRate={is_wr:.2%}, PF={is_pf:.2f}")
    print(f"  IBS trades: {is_result.get('ibs_trade_count', 'n/a')}, "
          f"TOM trades: {is_result.get('tom_trade_count', 'n/a')}")

    # ── 2. OOS Backtest ────────────────────────────────────────────────────────
    print(f"\n[2/8] OOS Backtest ({OOS_START} → {OOS_END})...")
    ...
    oos_returns   = np.array(oos_result["returns"].values
                             if isinstance(oos_result["returns"], pd.Series)
                             else oos_result["returns"])
    ...
    print(f"  OOS: Sharpe={oos_sharpe:.4f}, MDD={oos_mdd:.2%}, "
          f"Trades={oos_trades}, WinRate={oos_wr:.2%}, PF={oos_pf:.2f}")

    # ── 3. OOS Data Quality Validation ────────────────────────────────────────
    print("\n[3/8] OOS Data Quality Validation...")
    try:
        spy_oos = yf.download("SPY", start=OOS_START, end=OOS_END,
    ...
    oos_metrics_for_dq = {
        ...
        "post_cost_sharpe": oos_sharpe,   # approximate; refined below
        "total_return":     oos_return,
        "portfolio_returns": (oos_result["returns"]
                              if isinstance(oos_result["returns"], pd.Series)
                              else pd.Series(oos_returns)),
    }
    ...
    if dq_report["recommendation"] == "BLOCK":
        print(f"  [BLOCK] {dq_report.get('block_reasons', 'unknown')}")
        blocked_path = os.path.join(
            OUTPUT_DIR, f"{STRATEGY_NAME}_{TODAY}_blocked_dq.json"
        )
        with open(blocked_path, "w") as f:
            json.dump({"strategy": STRATEGY_NAME, "dq_report": dq_report},
    ...
    # Post-cost Sharpe (conservative drag estimate)
    ...
    post_cost_sharpe = round(is_sharpe - annual_drag * 5, 4)

    # Permutation test (alpha validation)
    ...
    hold_days_perm = int(PARAMETERS.get("max_hold_days", 3))
    perm = permutation_test_alpha(eq_prices, is_sharpe,
                                   n_perms=1000, hold_days=hold_days_perm)
    print(f"  Permutation test: p-value={perm['permutation_pvalue']:.4f}, "
          f"pass={perm['permutation_test_pass']}")

    # DSR (n_trials = 6 params × ~3 values each = ~18)
    dsr = compute_dsr(is_returns, n_trials=18)
    print(f"  DSR z-score={dsr:.4f}")

    # ── 5. Walk-Forward ───────────────────────────────────────────────────────
    print("\n[5/8] Walk-Forward (4 expanding windows)...")
    ...
    # ── 6. Sensitivity Scan ───────────────────────────────────────────────────
    print(f"\n[6/8] Sensitivity scan (IS {IS_START}→{IS_END})...")
    ...
    # ── 7. Regime Slices ─────────────────────────────────────────────────────
    print("\n[7/8] Regime-slice analysis...")
    regime_results = run_regime_slices(PARAMETERS.copy())

    # Regime criterion: ≥ 2 of 4 slices with Sharpe ≥ 0.8, ≥ 1 stress
    stress_regimes    = ["rate_shock_2022", "covid_crash_2020"]
    nonstress_regimes = ["pre_covid_2018_2019", "stimulus_era_2021"]
    stress_passing    = sum(1 for r in stress_regimes
                            if regime_results.get(r, {}).get("sharpe", 0.0) >= 0.8)
    nonstress_passing = sum(1 for r in nonstress_regimes
                            if regime_results.get(r, {}).get("sharpe", 0.0) >= 0.8)
    total_passing     = stress_passing + nonstress_passing
    regime_criterion_pass = stress_passing >= 1 and total_passing >= 2
    print(f"  Regime criterion (≥2/4 pass incl ≥1 stress): "
          f"{total_passing}/4 pass, {stress_passing}/2 stress → "
          f"{'PASS' if regime_criterion_pass else 'FAIL'}")

    # ── 8. Save Trade Log CSV ─────────────────────────────────────────────────
    trade_log_path = os.path.join(
        OUTPUT_DIR, f"{STRATEGY_NAME}_{TODAY}_trades.csv"
    )
    all_trades_dfs = []
    ...
    if all_trades_dfs:
        ...
        print(f"\n[8/8] Trade log saved: {trade_log_path} ({len(full_trades)} rows)")
    ...
    metrics = {
        ...
        "ibs_trade_count": int(is_result.get("ibs_trade_count", 0)),
        "tom_trade_count": int(is_result.get("tom_trade_count", 0)),
        ...
        "data_quality_notes": (
            "SPY/QQQ/IWM/VIX ETF universe. No survivorship bias. "
            "auto_adjust=True. SMA/ATR computed on warmup data outside backtest window."
        ),
        "gate1_pass": False,   # filled in after verdict
    }
    ...
    with open(verdict_txt_path, "w") as f:
        ...
        f.write(f"IS Sharpe:        {is_sharpe:.4f}\n")
        f.write(f"OOS Sharpe:       {oos_sharpe:.4f}\n")
        f.write(f"IS MDD:           {is_mdd:.2%}\n")
        f.write(f"OOS MDD:          {oos_mdd:.2%}\n")
        f.write(f"Win Rate (IS):    {is_wr:.2%}\n")
        f.write(f"Profit Factor:    {is_pf:.2f}\n")
        f.write(f"Trade Count (IS): {is_trades}\n")
        f.write(f"Trade Count (OOS):{oos_trades}\n")
        f.write(f"  IBS: {is_result.get('ibs_trade_count','n/a')}  "
                f"TOM: {is_result.get('tom_trade_count','n/a')}\n")
        f.write(f"Post-cost Sharpe: {post_cost_sharpe:.4f}\n")
        f.write(f"Monte Carlo p5:   {mc['mc_p5_sharpe']:.4f}\n")
        f.write(f"Bootstrap 95% CI: [{ci['sharpe_ci_low']:.3f}, {ci['sharpe_ci_high']:.3f}]\n")
        f.write(f"Permutation p-val:{perm['permutation_pvalue']:.4f}\n")
        f.write(f"WF Sharpe std/min:{wf_var['wf_sharpe_std']:.3f}/{wf_var['wf_sharpe_min']:.3f}\n")
        f.write(f"DSR z-score:      {dsr:.4f}\n")
        f.write(f"WF windows:       {wf['wf_windows_passed']}/4\n")
        f.write(f"Sensitivity:      pass={sens['sensitivity_pass']}, "
                f"max={sens['sensitivity_max_change_pct']:.2%}\n")
        f.write(f"Regime criterion: {'PASS' if regime_criterion_pass else 'FAIL'}\n")
        for rname, rd in regime_results.items():
            f.write(f"  {rname}: Sharpe={rd.get('sharpe',0.0):.3f}\n")
    ...
    print(f"Verdict TXT saved:  {verdict_txt_path}")
    print(f"Verdict JSON saved: {verdict_json_path}")

    # ── Summary ───────────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("GATE 1 VERDICT SUMMARY — H24 Combined IBS+TOM")
    print("=" * 70)
    print(f"Overall: {verdict['overall_verdict']} ({verdict['criteria_passed']} criteria)")
    print(f"IS Sharpe (2018–2023):  {is_sharpe:.4f}  (target > 1.0) — {'PASS' if is_sharpe > 1.0 else 'FAIL'}")
    print(f"OOS Sharpe (2024–2025): {oos_sharpe:.4f}  (target > 0.7) — {'PASS' if oos_sharpe > 0.7 else 'FAIL'}")
    ...
    print(f"Regime:     {'PASS' if regime_criterion_pass else 'FAIL'}")
    if verdict.get("failing_criteria"):
...
if __name__ == "__main__":
    ...
    try:
        result = main()
        if result[0] is None:
            print("\n[BLOCKED] OOS data quality check failed. See report above.")
            sys.exit(2)
    except VerdictValidationError as e:
        print(f"\n[VERDICT VALIDATION ERROR] {e}")
        traceback.print_exc()
        sys.exit(3)
