...
Date: 2026-03-16
Hypothesis: SPY RSI(2) < 10 in an uptrend (SPY > 200-SMA) marks extreme short-term
            oversold conditions caused by retail panic selling and dealer delta-hedging.
            Price mean-reverts to the 5-day SMA within 3-5 trading days.
...
References: Connors & Alvarez (2012) Short-Term Trading Strategies That Work;
            ...
            research/hypotheses/34_rsi2_oversold_spy_mean_reversion.md
"""
...
PARAMETERS = {
    ...
    "rsi_entry_threshold": 10,        # Enter when RSI(2) < this (test range: 5–15)
    ...
    "rearm_rsi_level": 40,            # RSI must cross above this after each exit before next entry
    "init_cash": 25000,
}
...
def compute_rsi(series: pd.Series, period: int = 2) -> pd.Series:
    ...
    At RSI(2) < 10, SPY has experienced 2+ consecutive sharp down days — behavioral
...
def simulate_h34(
    ...
    """
    Simulate H34 RSI(2) Oversold SPY Mean Reversion with 200-SMA regime filter.
    ...
    for i in range(n):
        ...
        # ── Step 5: Re-arm check ──────────────────────────────────────────────
        # After closing a position, wait for RSI to bounce above rearm_rsi_level (40)
        # before allowing the next entry. This prevents chasing a sustained decline.
        ...
        # RSI(2) < threshold, and SPY is above its 200-SMA (regime gate).
...
def run_backtest(start: str, end: str, params: dict = None) -> dict:
    ...
    series, trim to backtest window, and simulate H34.
    ...
    # ── 5. Simulate ───────────────────────────────────────────────────────────
    trade_log, equity, daily_df = simulate_h34(
    ...
    print(
        f"\nH34 RSI(2) Oversold SPY Backtest ({start} to {end}):\n"
...
def run_strategy(
    ...
    """
    Orchestrator-compatible entry point for H34.
    ...
    `ticker` is ignored — H34 uses SPY via PARAMETERS["ticker"].
    """
