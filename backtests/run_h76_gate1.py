"""
H76 Gate 1 Full Backtest Runner — Multi-ETF RSI-2 Daily Mean Reversion Portfolio
...
Parent: QUA-323 | Addresses: QUA-283 trade count failure (12 ETFs × daily RSI-2)
"""
...
from h76_multi_etf_rsi2_mean_reversion import (
...
STRATEGY_NAME = "H76_MultiETF_RSI2_DailyMeanReversion"
...
# Sweep: 9 combos — entry threshold × max positions
SWEEP_RSI_ENTRIES  = [3, 5, 7]
SWEEP_MAX_POSITIONS = [3, 4, 5]
...
def permutation_test_multi(
    ...
    """Random-entry permutation test across all 12 ETFs."""
    np.random.seed(44)
    permuted_sharpes = []
...
def compute_market_impact_all(tickers: list, is_start: str, is_end: str) -> dict:
    """Market impact estimate per ticker at 25%-weight sizing on $100K account."""
    results = {}
    typical_qty_by_price = {
        "SPY": 55, "QQQ": 60, "IWM": 145,
        "XLK": 105, "XLV": 95, "XLE": 155,
        "XLF": 450, "XLY": 100, "XLP": 225,
        "XLU": 200, "XLI": 185, "XLB": 210,
    }
    for t in tickers:
        try:
...
def main():
    print("=" * 70)
    print(f"H76 Gate 1 Backtest — {TODAY}")
    print(f"Universe: 12 ETFs (SPY/QQQ/IWM + 9 sector ETFs)")
    print(f"Signal: RSI(2) < 5 | Exit: RSI(2) > 65 OR 5-day max hold")
    print(f"Regime: SPY 200-DMA | Max positions: 4 | Equal weight 25%")
    print("=" * 70)
    ...
    print("  Running permutation test (1000 perms, 12-instrument)...")
    ...
    print(f"\n{'='*70}")
    print("GATE 1 SUMMARY — H76 Multi-ETF RSI-2 Portfolio:")
    ...
    # ── Save outputs ──────────────────────────────────────────────────────────
    base = f"H76_MultiETF_RSI2_{TODAY}"
    ...
    # Verdict text
    verdict_lines = [
        f"H76 Multi-ETF RSI-2 Daily Mean Reversion Portfolio — Gate 1 Verdict",
        f"Date: {TODAY}",
    ...
    verdict_lines += [
        ...
        f"  XLK/XLV/XLE/XLF/XLY/XLP/XLU/XLI/XLB: 0.05% one-way (canonical standard)",
...
<head>
<meta charset="utf-8">
<title>H76 Gate 1 Report — {TODAY}</title>
<style>
...
<body>
<h1>H76 Multi-ETF RSI-2 Daily Mean Reversion — Gate 1 Report</h1>
<p><strong>Date:</strong> {TODAY} | <strong>Task:</strong> QUA-323</p>
<p><strong>Universe:</strong> {', '.join(TICKERS)}</p>
<p><strong>Signal:</strong> RSI(2) &lt; 5 → buy next open | Exit: RSI(2) &gt; 65 OR 5-day max hold | SPY 200-DMA regime</p>
<p><strong>Sizing:</strong> Equal-weight 25% per position, max 4 concurrent | Stop: 5%</p>
...
<hr>
<p><em>Generated by Engineering Director (QUA-323) | H76 Gate 1 | {TODAY}</em></p>
</body>
</html>"""
