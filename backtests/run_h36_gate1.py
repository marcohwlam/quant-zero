"""
H36 Gate 1 Backtest Runner — Crypto Cross-Sectional Momentum (4-Asset)
...
CEO notes (from QUA-291 authorization):
  - Trade count < 50/year exception GRANTED (structural crypto IS window constraint)
  - Drawdown risk flagged: estimated 25-40% vs. Gate 1 <20% criterion
  - BTC trend filter must sufficiently constrain realized DD; if >20% → FAIL on drawdown
  - Correlation note: cross-sectional momentum on r>0.7 assets is architecturally distinct
    from mean-reversion pairs (QUA-281 criterion does not apply)

Output:
  backtests/H36_Crypto_Momentum_{TODAY}.json
  backtests/h36_crypto_momentum_gate1_report.md
"""
...
STRATEGY_NAME = "H36_Crypto_Momentum"
...
def main():
    np.random.seed(42)
    print("=" * 65)
    print(f"H36 Crypto Cross-Sectional Momentum — Gate 1 Backtest [{TODAY}]")
    print("CEO exception: <50 trades/year threshold waived (structural crypto IS window)")
    print("=" * 65)

    # ── 1. IS Backtest ────────────────────────────────────────────
    print(f"\n[1/6] Running IS backtest ({IS_START} to {IS_END})...")
    ...
    # ── 2. OOS Backtest ───────────────────────────────────────────
    print(f"\n[2/6] Running OOS backtest ({OOS_START} to {OOS_END})...")
    ...
    # ── 3. Walk-Forward ───────────────────────────────────────────
    print("\n[3/6] Running walk-forward analysis (4 windows, 36m IS / 6m OOS)...")
    wf_table = run_walk_forward(PARAMETERS, n_windows=4, is_months=36, oos_months=6)
    ...
    try:
        close_full, _ = download_data(
            ...
            PARAMETERS["ranking_window"], PARAMETERS["trend_sma_window"]
        )
    ...
    # 4e. DSR (n_trials = 5 ranking window + 4 hard stop = 9 combinations)
    n_trials = 9
    dsr = compute_dsr(is_returns, n_trials=n_trials) if len(is_returns) > 10 else 0.0
    print(f"  4e. DSR={dsr:.6f}")

    # ── 5. Sensitivity Scan ───────────────────────────────────────
    print("\n[5/6] Sensitivity scans (ranking window + hard stop)...")
    print("  5a. Ranking window scan [10, 15, 20, 25, 30] days...")
    rw_scan = scan_ranking_windows(IS_START, IS_END, PARAMETERS)
    ...
    stop_scan = scan_hard_stops(IS_START, IS_END, PARAMETERS)
    print(f"      {stop_scan}")

    # ── 6. Gate 1 Verdict ─────────────────────────────────────────
    print("\n[6/6] Computing Gate 1 verdict...")

    # Standard Gate 1 criteria
    gate1_checks = {
        "is_sharpe_pass": bool(is_sharpe > 1.0),
        "oos_sharpe_pass": bool(oos_sharpe > 0.7),
        "is_mdd_pass": bool(is_mdd > -0.20),          # <20% MDD (key risk flag from CEO)
        "oos_mdd_pass": bool(oos_mdd > -0.25),          # <25% OOS MDD
        "win_rate_pass": bool(is_win_rate >= 0.50 or is_profit_factor >= 1.2),
        "trade_count_pass": bool(is_trade_count >= 100),  # CEO exception: >=100 total (not 50/yr)
    ...
    metrics = {
        ...
        "ceo_flags": {
            "drawdown_risk": (
                "Research Director estimated 25-40% drawdown. Gate 1 criterion is <20%. "
                f"Measured IS MDD: {is_mdd:.1%}. "
                f"{'WITHIN Gate 1 limit.' if is_mdd > -0.20 else 'EXCEEDS Gate 1 <20% criterion — FAIL.'}"
            ),
            "correlation_note": (
    ...
    for w in wf_table:
        ...
        wf_rows += (
            ...
            f"| {w.get('is_trade_count','?')} | **{status}** |\n"
        )

    report_md = f"""# H36 Crypto Cross-Sectional Momentum — Gate 1 Report
...
**CEO Drawdown Flag:** Estimated 25–40% drawdown. {'BTC trend filter kept DD within Gate 1 limit.' if is_mdd > -0.20 else 'BTC trend filter INSUFFICIENT — DD exceeds Gate 1 <20% criterion.'}
...
| Test | Value | Status |
|---|---|---|
| DSR (n=9 trials) | {dsr:.6f} | {'PASS' if gate1_checks['dsr_pass'] else 'FAIL'} |
...
"""
    ...
    if gate1_pass:
        report_md += "All Gate 1 criteria passed. Strategy is eligible for paper trading (pending CEO approval).\n"
    else:
    ...
    report_path = "/mnt/c/Users/lamho/repo/quant-zero/backtests/h36_crypto_momentum_gate1_report.md"
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 65)
    print(f"H36 Gate 1 Final Verdict: {'PASS' if gate1_pass else 'FAIL'}")
    if failing:
